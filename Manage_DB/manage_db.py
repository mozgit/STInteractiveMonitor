...
def fill_random(nNentries=10):
    ...
    for i in range(0, nNentries):
        for name in NameList['TTNames']:
            st_sector(name = name, run = i, efficiency = random.random(), bias = (random.random()-0.5), width = random.random()/(i+1),
                                            err_efficiency = random.random()/10, err_bias = (random.random()-0.5)/10, err_width = 0).save()
...
def add_run(run_number, file):
    ...
    for st_id in TT_Map:
        ...
        st_sector(name = TT_Map[st_id], run = run_number, efficiency = hist_all_TT_Eff.GetBinContent(st_id+1), bias = hist.GetMean(), width = hist.GetRMS(),
                                                          err_efficiency = hist_all_TT_Eff.GetBinError(st_id+1), err_bias = hist.GetRMS(), err_width = 0).save()
    for st_id in IT_Map:
        hist = hist_all_IT_Mon.ProjectionY(IT_Map[st_id], st_id+1, st_id+1)
        random.seed(datetime.datetime.now().microsecond)
        st_sector(name = IT_Map[st_id], run = run_number, efficiency = hist_all_IT_Eff.GetBinContent(st_id+1), bias = hist.GetMean(), width = hist.GetRMS(),
                                                          err_efficiency = hist_all_IT_Eff.GetBinError(st_id+1), err_bias = hist.GetRMS(), err_width = 0).save()
