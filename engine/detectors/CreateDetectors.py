...
def doallthethings(name, Drawing_Mode, db):
    ...
    trend = {}
    for document in db.st_sector.find({"name":name,
                                        #"run":{"$gt":int(start_run)-1},
                                        "$or": [ { "run":{"$gt":int(start_run)}  }, {"run":start_run} ],
                                        "run":{"$lt":int(end_run)+1}}):
...
def create_TT(Drawing_Mode = {'hist':'efficiency', 'prop':'mean', 'start_run':0, 'end_run':3}):
...
def create_IT(Drawing_Mode = {'hist':'efficiency', 'prop':'mean', 'start_run':0, 'end_run':3}):
    f = open('engine/NameList.pkl')
    NameList = pickle.load(f) 
    client = connect('st_db')
    db = client.st_db
